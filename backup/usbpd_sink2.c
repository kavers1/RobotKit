 ...
 */
int getchar( void )  // kov for serial input should go
{
...
/// void PD_update(void) {
	...
	while ( eUSBPD_BUSY == ( result = USBPD_SinkNegotiate() ) )
	{
		...
		if ( s_systickCount > ( lastLog + 1000 ) )  // log output every second
		{
			LOG( "Negotiating USB PD: %s", USBPD_StateToStr( USBPD_GetState() ) );
			lastLog = s_systickCount;
		}
		// if not in debug mode we can not use character commands
