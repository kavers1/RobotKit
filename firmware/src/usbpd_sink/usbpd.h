...
/// @brief get the maximum voltage the PDO/PPS can deliver
/// @param i the index of the selected PDO
/// @return max voltage in mV
uint32_t get_MaxVoltageInmV(int i){
	/// TODO what if no PDO's available, we shouldn't be here since the communication failure status flag should be set
	if (i > s_instance.pdoCount) i = 1;
	const USBPD_SourcePDO_t *pdo = &capabilities->Source[i];
	switch ( pdo->Header.PDOType )
	{

	case eUSBPD_PDO_FIXED: 
			return ( ( pdo )->FixedSupply.VoltageIn50mV * 50 );
		break;		
	case eUSBPD_PDO_BATTERY: 
			return ( ( pdo )->BatterySupply.MaxnVoltageIn50mV * 50 ) ;
		break;
	case eUSBPD_PDO_VARIABLE:
			return ( ( pdo )->VariableSupply.MaxVoltageIn50mV * 50 );
		break;
	case eUSBPD_PDO_AUGMENTED:
		switch ( pdo->Header.AugmentedType )
		{
		case eUSBPD_APDO_SPR_PPS: 
				return ( ( pdo )->SPR_PPS.MaxVoltageIn100mV * 100 ); 
			break;
		case eUSBPD_APDO_SPR_AVS: 
		/// TODO do we limit this to 15V or 18V ??
				return = 20; 
			break;
		case eUSBPD_APDO_EPR_AVS:
				return ( ( pdo )->EPR_AVS.MaxVoltageIn100mV * 100 ); 
			break;
		}
		default : return 5000; // standard 5V 
		break;
	}
}
#endif
